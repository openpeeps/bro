...
proc initCSS*(script: Script, systemModule: Module): Module =
  ...
  # CSS Functions
  #
  script.addProc(result, "url", @[paramDef("s", tyString)], tyString,
  ...
  # CSS Properties
  #
  script.addProc(result, "background-color", @[paramDef("s", tyString)], tyString,
    proc (args: StackView): Value =
      discard parseHtmlColor(args[0].stringVal[])
      args[0]
  )

  script.addProc(result, "background-image", @[paramDef("s", tyString)], tyString,
    proc (args: StackView): Value =
      args[0]
  )

  script.addProc(result, "background-origin", @[paramDef("s", tyString)], tyString,
    proc (args: StackView): Value =
  ...
  script.addProc(result, "background-position-x", @[paramDef("s", tyString)], tyString,
    proc (args: StackView): Value =
      args[0]
  )


  script.addProc(result, "background-position-y", @[paramDef("s", tyString)], tyString,
    proc (args: StackView): Value =
      args[0]
  )

  script.addProc(result, "background-position", @[paramDef("x", tyString), paramDef("y", tyString)], tyString,
    proc (args: StackView): Value =
      initValue(args[0].stringVal[] & " " & args[1].stringVal[])
  )

  script.addProc(result, "background-repeat", @[paramDef("s", tyString)], tyString,
    proc (args: StackView): Value =
  ...
  script.addProc(result, "color", @[paramDef("s", tyString)], tyString,
    proc (args: StackView): Value =
      discard parseHtmlColor(args[0].stringVal[])
      args[0]
  )